...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Cat Domain Names", "CDN") {
    owner = payable(msg.sender);
    tld = _tld;
    console.log("%s name service deplyed", _tld);
  }
  ...
  function price(string calldata name) public view returns(uint) {
    ...
    if (len == 3) {
      ...
      return 3 * 10**17; // to charge smaller amounts reduct the decimals. This is .3
    } else {
      return 1 * 10**17;
    }
  }
  ...
  function register(string calldata name) public payable {
    //Check the the name is unregistered 
